...
function Inner() {
  ...
  const changeRoleMutation = useMutation({
    ...
    onSuccess: (account) => {
      setNotice(`${account.username} 角色已更新为 ${ROLE_LABELS[account.role] ?? account.role}。`);
      setFormError(null);
      void queryClient.invalidateQueries({ queryKey: ["platform-accounts"] });
    },
    onError: (error) =>
  ...
  const busy = create.isPending || setStatus.isPending || changeRoleMutation.isPending;
        ...
        <>
          <div className="pw-page-head">
            <div>
              ...
              <h1>平台账号</h1>
              <p>平台管理员与运营（support）权限管理，每次变更写入平台审计。</p>
            </div>
          </div>
                      ...
                      return (
                        <tr key={account.id}>
                          <td>
                            ...
                            {self ? (
                              <span className="pw-pending" style={{ marginLeft: 8 }}>
                                当前账号
                              </span>
                            ) : null}
                          </td>
                          ...
                          <td>
                            {self ? (
                              <span style={{ color: "var(--pw-muted)", fontSize: 11 }}>
                                不可操作
                              </span>
                            ) : (
                              <button
          ...
          <div className="pw-notice">
            平台运营（support）默认不能查看订单截图与财务明细；跨租户临时授权将在 P-B2b 落地。
          </div>
        </>
      )}
    </PlatformShell>
