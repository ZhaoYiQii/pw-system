  ...
  return (
    <div>
      <MonitorHeader
        kicker="MONITOR / LIVE"
        title="进行中场次"
        description="服务端计时中的场次。"
      >
        <Link href="/merchant-console/sessions" className="mc-btn">
          全部场次
        </Link>
      </MonitorHeader>
      <section className="mc-panel">
        {rows.length ? (
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>
                      <Link
                        href={`/merchant-console/sessions/${row.id}`}
                        className="mc-btn mc-btn-ghost mc-btn-small"
                      >
                        盯场 <ArrowRight size={13} />
                      </Link>
                    </td>
                  </tr>
                ))}
          ...
          <div className="mc-empty">加载中…</div>
        ) : (
          <DemoEmptyState
            title="暂无进行中场次"
            description="场次开始后出现在这里。"
          />
        )}
      </section>
    </div>
              ...
              <tbody>
                {adjustments.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      <Link
                        href={`/merchant-console/sessions/${s.id}`}
                        className="mc-btn mc-btn-ghost mc-btn-small"
                      >
                        复核 <ArrowRight size={13} />
                      </Link>
                    </td>
                  </tr>
                ))}
  ...
  return (
    <div>
      <MonitorHeader
        ...
        title="通知与任务健康"
        description="站内通知已读管理；短信/微信通道为外部挂起项。"
      ></MonitorHeader>
      <section className="mc-panel mc-channel-panel">
        ...
        <div className="mc-channel-list">
          <div className="mc-channel-row">
            <span>
              <b>站内通知</b>
              <small>未读 {unread.data?.count ?? 0} 条</small>
            </span>
            <span className="mc-status st-done">正常</span>
          </div>
          <div className="mc-channel-row">
            <span>
              <b>短信 / 微信</b>
              <small>当前仓库未接入外部通道</small>
            </span>
            <span className="mc-status st-muted">未配置</span>
          </div>
        </div>
      </section>
      <section className="mc-panel">
