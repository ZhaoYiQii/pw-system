...
import { ArrowLeft, Check, Image, Plus, UserPlus, X } from "lucide-react";
...
function LoginAccountSetup({
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const account = await apiFetch<TenantAccount>("/api/v1/tenant/accounts", {
        method: "POST",
        body: JSON.stringify({
    ...
    <div>
      ...
      <section className="mc-panel mc-facts">
        <dl className="mc-fact">
          <dt>客户 ID</dt>
          <dd className="mc-mono">{customer.id}</dd>
        </dl>
        <dl className="mc-fact">
          <dt>备注</dt>
              ...
              <tbody>
                {wallet.entries.map((entry) => (
                  <tr key={entry.id}>
                    <td>{entry.txNo}</td>
                    <td>{entry.type}</td>
                    <td>{formatFenYuan(entry.amountFen)}</td>
                    <td>{formatFenYuan(entry.balanceAfterFen)}</td>
                    <td>{entry.reason ?? "-"}</td>
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          ...
          <span>{orderHistory.length} 单</span>
        </div>
        {orderHistory.length ? (
          <div className="mc-table-wrap">
            <table>
              ...
              <tbody>
                {orderHistory.map((order) => (
                  <tr key={order.orderId}>
                    ...
                    <td>
                      <Link
                        href={`/merchant-console/dispatch/${order.orderId}?kind=${order.processType === "GAME_DISPATCH" ? "GD" : "CLASSIC"}`}
...
function PlayerDetailView({ id }: { id: string }) {
  ...
  const toggleAccept = useMutation({
    mutationFn: () =>
      apiFetch<PlayerRow>(`/api/v1/tenant/players/${id}`, {
        method: "PATCH",
        body: JSON.stringify({
          acceptingOrders: !detailQuery.data?.acceptingOrders,
        }),
      }),
    onSuccess: () => {
      refresh();
  ...
  if (detailQuery.isError || !detailQuery.data)
    return <div className="mc-empty">陪玩不存在或无权访问。</div>;
    ...
    <div>
      ...
      <div className="mc-pagehead mc-detail-head">
        <div>
          ...
          <p>
            {player.mobile ?? "未填写手机"} · 基础小时价{" "}
            {formatFenYuan(player.basePricePerHourFen)}
          </p>
        </div>
            ...
            <table>
              ...
              <tbody>
                {player.availability.map((slot) => (
                  <tr key={slot.id}>
                    <td>{dateTime(slot.startsAt)}</td>
                    <td>{dateTime(slot.endsAt)}</td>
                    <td>{slot.reason ?? "-"}</td>
                    <td>
                      <button
                        type="button"
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          <div>
            <h2>收入概览</h2>
            <p>
              已付 {account ? formatFenYuan(account.finance.paidFen) : "-"} ·
              未付 {account ? formatFenYuan(account.finance.unpaidFen) : "-"}
            </p>
          </div>
        </div>
              ...
              <tbody>
                {account!.finance.records.map((record) => (
                  <tr key={record.id}>
                    ...
                    <td>{record.source}</td>
                    <td>{formatFenYuan(record.amountFen)}</td>
                    <td>
...
function SessionDetailView({ id }: { id: string }) {
  ...
  if (query.isError || !query.data)
    return <div className="mc-empty">场次不存在或无权访问。</div>;
  const session = query.data;
  const canStart = ["ASSIGNED", "READY", "NOT_STARTED", "SCHEDULED"].includes(
    session.status,
  );
    ...
    <div>
      ...
      <div className="mc-pagehead mc-detail-head">
        <div>
          ...
          <div className="mc-detail-title">
            <h1 className="mc-mono">{session.id}</h1>
            <Badge status={session.status} />
          </div>
          <p>
            {session.flow} · 订单 {session.orderId}
          </p>
        </div>
        <div className="mc-button-row">
          {canStart ? (
          ...
          <Link
            href={`/merchant-console/dispatch/${session.orderId}?kind=${session.flow === "GAME_DISPATCH" ? "GD" : "CLASSIC"}`}
            className="mc-btn"
          >
            订单详情
          </Link>
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          ...
          <span>{session.evidence.length} 份</span>
        </div>
            ...
            <table>
              ...
              <tbody>
                {session.adjustments.map((adj) => (
                  <tr key={adj.id}>
                    <td>{formatDuration(adj.originalDurationSeconds)}</td>
                    <td>{formatDuration(adj.requestedDurationSeconds)}</td>
                    <td>{adj.reason}</td>
                    <td>
...
function SettlementDetail({ id }: { id: string }) {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiFetch<SettlementBatchDetail>(`/api/v1/tenant/settlements/${id}`),
  });
  const refresh = () =>
    void queryClient.invalidateQueries({
      queryKey: ["merchant", "settlement-detail", id],
  ...
  if (query.isError || !query.data)
    return <div className="mc-empty">批次不存在或无权访问。</div>;
    ...
    <div>
      ...
      <div className="mc-pagehead mc-detail-head">
        <div>
          ...
          <div className="mc-detail-title">
            <h1 className="mc-mono">{batch.batchNo}</h1>
            <Badge status={batch.status} />
          </div>
          <p>
            总额 {formatFenYuan(batch.totalAmountFen)} · {batch.itemCount} 笔
          </p>
        </div>
        <div className="mc-button-row">
          {nextAction && batch.status === "DRAFT" ? (
            <button
              type="button"
              className="mc-btn mc-btn-primary"
              onClick={() => run.mutate(nextAction)}
            >
              复核
            </button>
          ) : null}
          {nextAction && batch.status === "REVIEWED" ? (
            <button
              type="button"
              className="mc-btn mc-btn-primary"
              onClick={() => run.mutate(nextAction)}
          ...
          {nextAction && batch.status === "APPROVED" ? (
            <button
              type="button"
              className="mc-btn mc-btn-primary"
              onClick={() => run.mutate(nextAction)}
          ...
          {["DRAFT", "REVIEWED", "APPROVED"].includes(batch.status) ? (
            <button
              type="button"
              className="mc-btn"
              onClick={() => run.mutate("void")}
            >
              作废
            </button>
          ) : null}
      ...
      <section className="mc-panel mc-facts">
        ...
        <dl className="mc-fact">
          <dt>复核人</dt>
          <dd>{batch.reviewedByName ?? "-"}</dd>
        </dl>
        <dl className="mc-fact">
          <dt>批准人</dt>
          <dd>{batch.approvedByName ?? "-"}</dd>
        </dl>
        <dl className="mc-fact">
          <dt>支付人/时间</dt>
          <dd>
            {batch.paidByName ?? "-"}{" "}
            {batch.paidAt ? `· ${dateTime(batch.paidAt)}` : ""}
          </dd>
        </dl>
      </section>
      <section className="mc-panel">
        <div className="mc-section-head">
          ...
          <span>{batch.items.length} 笔</span>
        </div>
        {batch.items.length ? (
          <div className="mc-table-wrap">
            <table>
              ...
              <tbody>
                {batch.items.map((item) => (
                  <tr key={item.itemId}>
                    ...
                    <td>{item.source}</td>
                    <td>{formatFenYuan(item.amountFen)}</td>
...
function DisputeDetailView({ id }: { id: string }) {
  ...
  if (query.isError || !query.data)
    return <div className="mc-empty">争议不存在或无权访问。</div>;
    ...
    <div>
      ...
      <div className="mc-pagehead mc-detail-head">
        <div>
          ...
          <div className="mc-detail-title">
            <h1 className="mc-mono">{dispute.orderNo}</h1>
      ...
      <section className="mc-panel mc-facts">
        ...
        <dl className="mc-fact">
          <dt>处理人/结论</dt>
          <dd>
            {dispute.resolvedBy ?? "-"} · {dispute.resolution ?? "未处理"}
          </dd>
        </dl>
      </section>
      {dispute.earning ? (
        <section className="mc-panel mc-facts">
          <dl className="mc-fact">
            <dt>应收金额</dt>
            <dd>{formatFenYuan(dispute.earning.amountFen)}</dd>
          </dl>
          <dl className="mc-fact">
            <dt>应收状态</dt>
            <dd>
              <Badge status={dispute.earning.status} />
            </dd>
          </dl>
          <dl className="mc-fact">
            <dt>结算批次</dt>
            <dd>
              {dispute.earning.settlementBatchNo ?? "未入批次"}{" "}
              {dispute.earning.settlementBatchStatus ?? ""}
            </dd>
          </dl>
        </section>
      ) : null}
          ...
          <ol className="mc-timeline">
            {dispute.events.map((event) => (
              <li key={event.id}>
                ...
                <b>{event.eventType}</b>
                <p>
                  {event.actorType ?? "-"} · {event.fromStatus ?? "—"} →{" "}
                  {event.toStatus ?? "—"}
                </p>
              </li>
            ))}
