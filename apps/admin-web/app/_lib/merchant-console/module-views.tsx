...
export function PlayerApplicationsModuleView() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiFetch<PlayerApplicationRow[]>("/api/v1/tenant/player-applications"),
  });
...
export function Badge({ status }: { status: string }) {
  return (
    <span className={`mc-status st-${toneFor(status)}`}>
      {statusLabel(status)}
    </span>
  );
}
  ...
  return (
    <div>
      <ModuleHeader
        ...
        title="服务目录"
        description="游戏、区服、服务产品与计价规则的真实配置入口。"
      >
        <Link href="/merchant-console/customers" className="mc-btn">
          客户档案
        </Link>
      </ModuleHeader>
      {error ? <div className="mc-notice">{error}</div> : null}
          ...
          <section className="mc-panel">
            ...
            <div className="mc-form-grid">
              ...
              <button
                type="button"
                className="mc-btn mc-btn-primary"
                disabled={!selectedProductId || !ruleDuration || !rulePrice}
                ...
                <tbody>
                  {games.map((game) => (
                    <tr key={game.id}>
                      ...
                      <td>
                        <Badge status={game.enabled ? "ACTIVE" : "INACTIVE"} />
                      </td>
                      <td>
                        <button
                          type="button"
  ...
  return (
    <div>
      <ModuleHeader
        ...
        title="场次与证据"
        description="CLASSIC 与 GD 场次统一台账：状态、计时、证据与待复核调整。"
      >
        <Link href="/merchant-console/live" className="mc-btn">
          进行中场次
        </Link>
      </ModuleHeader>
      <section className="mc-panel">
        ...
        {rows.length ? (
          ...
          <div className="mc-empty">加载场次中…</div>
        ) : (
          <DemoEmptyState
            title="暂无场次"
            description="订单开始服务后自动生成。"
          />
        )}
      </section>
    </div>
      ...
      {rules ? (
        <section className="mc-panel mc-form-panel">
          ...
          <div className="mc-summary-line">
            <span>平台费率</span>
            <b>{(rules.platformFeeBp / 100).toFixed(2)}%</b>
          </div>
          <div className="mc-form-grid">
            <label className="mc-field">
              <span>门店抽成（基点）</span>
              <input
                ...
                placeholder={String(rules.storeCutBp)}
              />
            </label>
            <button
              type="button"
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          <div>
            <h2>应收账本</h2>
            <p>
              已付 {ledger ? formatFenYuan(ledger.paidFen) : "-"} · 未付{" "}
              {ledger ? formatFenYuan(ledger.unpaidFen) : "-"}
            </p>
          </div>
        </div>
        {(ledger?.rows ?? []).length ? (
              ...
              <tbody>
                {ledger!.rows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.source}</td>
                    <td>{formatFenYuan(row.amountFen)}</td>
                    <td>
                      <Badge status={row.status} />
                    </td>
                    <td>{row.batchNo ?? "-"}</td>
  ...
  return (
    <div>
      <ModuleHeader
        ...
        title="结算批次"
        description="批次状态机：生成、复核、批准、登记支付与冲正。"
      >
        <Link href="/merchant-console/finance" className="mc-btn">
          收入账本
        </Link>
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          ...
          <button
            type="button"
            className="mc-btn mc-btn-primary"
            disabled={pendingIds.length === 0 || addToBatch.isPending}
              ...
              <tbody>
                {earnings.map((e) => (
                  <tr key={e.id}>
                    ...
                    <td>{e.playerName}</td>
                    <td>{e.source}</td>
                    <td>{formatFenYuan(e.amountFen)}</td>
      ...
      <section className="mc-panel">
        <div className="mc-section-head">
          <div>
            <h2>批次列表</h2>
            <p>进入详情执行复核、批准、支付与作废。</p>
          </div>
          <span>{rows.length} 批</span>
        </div>
        {rows.length ? (
          <div className="mc-table-wrap">
            <table>
              ...
              <tbody>
                {rows.map((batch) => (
                  <tr key={batch.id}>
                    <td>
                      <Link
                        href={`/merchant-console/settlements/${batch.id}`}
                        className="mc-order-link"
                      >
                        {batch.batchNo}
                      </Link>
  ...
  return (
    <div>
      <ModuleHeader
        ...
        title="客诉记录"
        description="争议证据、处理时间线、结算冻结状态集中查看。"
      >
        <Link href="/merchant-console/risk" className="mc-btn">
          异常与争议
        </Link>
      </ModuleHeader>
      <section className="mc-panel">
        <div className="mc-filterbar">
          <span className="mc-muted-text">共 {rows.length} 条</span>
        </div>
        {rows.length ? (
          <div className="mc-table-wrap">
            <table>
              ...
              <tbody>
                {rows.map((d) => (
                  <tr key={d.id}>
                    ...
                    <td>
                      <Link
                        href={`/merchant-console/disputes/${d.id}`}
                        className="mc-btn mc-btn-ghost mc-btn-small"
                      >
                        处理 <ArrowRight size={13} />
                      </Link>
                    </td>
                  </tr>
                ))}
            ...
            <table>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id}>
                    <td>
                      <b className="mc-mono">{row.action}</b>
                    </td>
                    <td>
