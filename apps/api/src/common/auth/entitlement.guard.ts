...
export const RequireAddon = (featureKey: string) => Reflect.metadata(REQUIRED_ADDON_KEY, featureKey);
...
export class EntitlementGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const feature = this.reflector.getAllAndOverride<string>(REQUIRED_ADDON_KEY, [
      context.getHandler(),
      context.getClass()
    ]);
