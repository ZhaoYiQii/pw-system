 ...
 */
export function tenantGuarded<Repo>(
  client: PrismaClient,
  repo: Repo,
  tenantIndex = 0,
): Repo {
      ...
      return (...args: unknown[]): unknown => {
        ...
        const tenantId =
          typeof raw === "string" && raw.length > 0
            ? raw
            : argRecord !== null &&
                typeof argRecord.tenantId === "string" &&
                argRecord.tenantId.length > 0
              ? argRecord.tenantId
              : undefined;
        ...
        return withTenantContext(
          ...
          async (tx: DbTransaction) => {
            ...
            const txLike = new Proxy(tx, {
              get(target, prop) {
                if (prop === "$transaction") {
                  return (fn: (inner: DbTransaction) => Promise<unknown>) =>
                    fn(txLike as unknown as DbTransaction);
                }
            ...
            const scoped = Object.create(obj) as RepoLike &
