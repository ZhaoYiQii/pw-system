...
export class AiAssistantService {
  ...
  async parseRequirement(tenantId: string, actorId: string, fields: RequirementFields) {
    ...
    const run = await this.client.aiRun.create({
    ...
    await this.client.aiSuggestion.create({
      data: {
        tenantId,
        runId: run.id,
  ...
  async recommendPlayers(tenantId: string, actorId: string, orderId: string) {
    ...
    const order = await this.client.order.findFirst({ where: { tenantId, id: orderId }, select: { id: true, orderNo: true, status: true } });
    ...
    if (req.serviceProductId) {
      const p = await this.client.serviceProduct.findFirst({ where: { tenantId, id: req.serviceProductId }, select: { gameId: true } });
      gameId = p?.gameId ?? null;
    }
    let players = await this.client.playerProfile.findMany({
      where: { tenantId, acceptingOrders: true, status: "ACTIVE" },
      select: { id: true, name: true, acceptingOrders: true },
      orderBy: { createdAt: "asc" }
    });
    if (gameId) {
      const skills = await this.client.playerSkill.findMany({ where: { tenantId, gameId }, select: { playerId: true } });
