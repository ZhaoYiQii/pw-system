...
function assertMoneyFen(
  ...
  if (fen === null) {
    throw new InvalidCatalogInputError(
      `${label} 必须为${allowZero ? "非负" : "正"}整数十进制字符串（分）`,
    );
  }
  return fen;
}
...
export class CatalogService {
  ...
  async createRegion(
    ...
    if (!game) throw new CatalogNotFoundError("game", gameId);
    try {
      return await this.repository.createRegion(
        tenantId,
        gameId,
        assertName(name, "区服名"),
      );
  ...
  async updateProduct(
    ...
    if (input.name !== undefined)
      clean.name = assertName(input.name, "产品名", 80);
    ...
    if (input.gameRegionId !== undefined)
      clean.gameRegionId = (input.gameRegionId as string | null) ?? null;
  ...
  async createRule(
    ...
    const cost =
      input.playerCostFen === undefined
        ? "0"
        : assertMoneyFen(input.playerCostFen, "playerCostFen", true);
    try {
  ...
  async updateRule(
    ...
    if (input.durationSeconds !== undefined)
      clean.durationSeconds = assertDurationSeconds(input.durationSeconds);
    if (input.priceFen !== undefined)
      clean.priceFen = assertMoneyFen(input.priceFen, "priceFen", false);
    if (input.playerCostFen !== undefined)
      clean.playerCostFen = assertMoneyFen(
        input.playerCostFen,
        "playerCostFen",
        true,
      );
