...
function isP2003(error: unknown): boolean {
  return error !== null && typeof error === "object" && "code" in error && (error as { code?: string }).code === "P2003";
}
...
function mapGame(row: PlainGame): GameView {
  return { id: row.id, tenantId: row.tenantId, name: row.name, enabled: row.enabled, createdAt: row.createdAt, updatedAt: row.updatedAt };
}

function mapRegion(row: PlainRegion): RegionView {
  return { id: row.id, tenantId: row.tenantId, gameId: row.gameId, name: row.name, enabled: row.enabled, createdAt: row.createdAt, updatedAt: row.updatedAt };
}
...
async function enrichProducts(client: PrismaClient, rows: PlainProduct[]): Promise<ProductView[]> {
  if (rows.length === 0) return [];
  const gameIds = Array.from(new Set(rows.map((r) => r.gameId)));
  const regionIds = Array.from(new Set(rows.filter((r) => r.gameRegionId !== null).map((r) => r.gameRegionId as string)));
  const games = await client.game.findMany({ where: { id: { in: gameIds } }, select: { id: true, name: true } });
  const regions = regionIds.length > 0 ? await client.gameRegion.findMany({ where: { id: { in: regionIds } }, select: { id: true, name: true } }) : [];
...
export class PrismaCatalogRepository implements CatalogRepository {
  ...
  async createGame(tenantId: string, name: string): Promise<GameView> {
    try {
      return mapGame(await this.client.game.create({ data: { tenantId, name } }));
    } catch (error) {
      if (isP2002(error)) throw new DuplicateCatalogEntryError(`game 已存在：${name}`);
      throw error;
    }
  }

  async updateGame(tenantId: string, id: string, input: { name?: string; enabled?: boolean }): Promise<GameView | null> {
    try {
  ...
  async removeGame(tenantId: string, id: string): Promise<boolean> {
    try {
      const res = await this.client.game.deleteMany({ where: { tenantId, id } });
      return res.count > 0;
    } catch (error) {
  ...
  async createRegion(tenantId: string, gameId: string, name: string): Promise<RegionView> {
    try {
      return mapRegion(await this.client.gameRegion.create({ data: { tenantId, gameId, name } }));
    } catch (error) {
      if (isP2002(error)) throw new DuplicateCatalogEntryError(`区服已存在：${name}`);
      if (isP2003(error)) throw new CatalogInUseError("game", gameId);
      throw error;
    }
  }

  async updateRegion(tenantId: string, id: string, input: { name?: string; enabled?: boolean }): Promise<RegionView | null> {
    try {
      ...
      if (res.count === 0) return null;
      const row = await this.client.gameRegion.findFirst({ where: { tenantId, id } });
  ...
  async removeRegion(tenantId: string, id: string): Promise<boolean> {
    try {
      const res = await this.client.gameRegion.deleteMany({ where: { tenantId, id } });
      return res.count > 0;
    } catch (error) {
  ...
  async listProducts(tenantId: string, opts: { gameId?: string; enabled?: boolean }): Promise<ProductView[]> {
    ...
    const rows = await this.client.serviceProduct.findMany({ where, orderBy: { createdAt: "asc" } });
    return enrichProducts(this.client, rows);
  }

  async findProduct(tenantId: string, id: string): Promise<ProductView | null> {
    const row = await this.client.serviceProduct.findFirst({ where: { tenantId, id } });
    if (!row) return null;
    const views = await enrichProducts(this.client, [row]);
    return views[0] as ProductView;
  }

  async createProduct(tenantId: string, input: { gameId: string; gameRegionId?: string | null; name: string; description?: string | null }): Promise<ProductView> {
    try {
      ...
      if (isP2002(error)) throw new DuplicateCatalogEntryError("同名产品已存在");
      if (isP2003(error)) throw new CatalogInUseError("game/region", input.gameId);
      throw error;
    }
  }

  async updateProduct(tenantId: string, id: string, input: { name?: string; description?: string | null; enabled?: boolean; gameRegionId?: string | null }): Promise<ProductView | null> {
    try {
      const res = await this.client.serviceProduct.updateMany({
        ...
        data: {
          ...
          ...(input.enabled !== undefined ? { enabled: input.enabled } : {}),
          ...(input.gameRegionId !== undefined ? { gameRegionId: input.gameRegionId } : {})
        }
      });
      if (res.count === 0) return null;
  ...
  async removeProduct(tenantId: string, id: string): Promise<boolean> {
    try {
      const res = await this.client.serviceProduct.deleteMany({ where: { tenantId, id } });
      return res.count > 0;
    } catch (error) {
  ...
  async listRules(tenantId: string, productId?: string): Promise<PricingRuleView[]> {
    const rows = await this.client.pricingRule.findMany({
      where: { tenantId, ...(productId ? { serviceProductId: productId } : {}) },
  ...
  async createRule(tenantId: string, productId: string, input: { durationSeconds: number; priceFen: MoneyFen; playerCostFen: MoneyFen; enabled: boolean }): Promise<PricingRuleView> {
    try {
      ...
      if (isP2002(error)) throw new DuplicateCatalogEntryError("该时长价格已存在");
    ...
    try {
      const res = await this.client.pricingRule.updateMany({
        ...
        data: {
          ...
          ...(input.priceFen !== undefined ? { priceFen: BigInt(input.priceFen) } : {}),
          ...(input.playerCostFen !== undefined ? { playerCostFen: BigInt(input.playerCostFen) } : {}),
          ...(input.enabled !== undefined ? { enabled: input.enabled } : {})
        }
      });
      if (res.count === 0) return null;
      const row = await this.client.pricingRule.findFirst({ where: { tenantId, id } });
  ...
  async removeRule(tenantId: string, id: string): Promise<boolean> {
    const res = await this.client.pricingRule.deleteMany({ where: { tenantId, id } });
    return res.count > 0;
  }
}
