...
export class DispatchPlayerController {
  ...
  private mapError(error: unknown): never {
    ...
    if (error instanceof OrderStateConflictError || error instanceof ApplicationConflictError) throw new HttpException(error.message, HttpStatus.CONFLICT);
  ...
  async mine(@Req() req: AuthenticatedRequest) {
    ...
    try {
      ...
      return { data: await this.dispatch.myApplications(ctx.tenantId, player.id) };
    } catch (error) {
  ...
  async apply(@Req() req: AuthenticatedRequest, @Param("orderId") orderId: string, @Body() body: { note?: unknown }) {
    const ctx = this.ctx(req);
    try {
      const player = await this.players.getByAccount(ctx.tenantId, ctx.accountId);
