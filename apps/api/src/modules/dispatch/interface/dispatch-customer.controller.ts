...
import { ApplicationConflictError, AssignmentExistsError, DispatchNotFoundError, OrderStateConflictError } from "../domain/errors.js";
...
export class DispatchCustomerController {
  ...
  private async ctxOf(req: AuthenticatedRequest, orderId: string): Promise<{ tenantId: string; accountId: string; customerProfileId: string }> {
    ...
    const profile = await this.customers.getByAccount(p.tenantId, p.sub).catch(() => {
      throw new ForbiddenException("尚未绑定客户档案");
    });
    const order = await this.orders.get(p.tenantId, orderId).catch(() => {
      throw new HttpException("订单不存在", HttpStatus.NOT_FOUND);
    });
    if (order.customerProfileId !== profile.id) throw new ForbiddenException("只能查看/操作自己的订单");
    return { tenantId: p.tenantId, accountId: p.sub, customerProfileId: profile.id };
  }
  ...
  async candidates(@Req() req: AuthenticatedRequest, @Param("orderId") orderId: string) {
