...
export class PrismaDispatchRepository implements DispatchRepository {
  ...
  async publish(
    ...
    await this.client.$transaction(async (tx) => {
      ...
      const existing = await tx.dispatchPublication.findFirst({
        where: { tenantId, orderId },
        select: { id: true, status: true },
      });
      if (existing) {
        if (existing.status !== "OPEN") {
          await tx.dispatchPublication.update({
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          payload: { orderId, orderNo: order.order_no },
        },
      });
    });
  }
  ...
  private async verifyPlayer(
    ...
    const player = await this.client.playerProfile.findFirst({
      ...
      select: { acceptingOrders: true, status: true },
    });
    if (!player || player.status !== "ACTIVE")
      throw new DispatchNotFoundError("player", playerId);
    if (!player.acceptingOrders) throw new PlayerNotAcceptingError(playerId);
  }

  private async verifySkillAndTime(
    tenantId: string,
    orderId: string,
    playerId: string,
  ): Promise<void> {
    const req = await this.client.orderRequirement.findFirst({
      where: { tenantId, orderId },
    });
    if (!req) throw new DispatchNotFoundError("requirement", orderId);
    let gameId: string | null = null;
    if (req.serviceProductId) {
      const product = await this.client.serviceProduct.findFirst({
    ...
    if (gameId) {
      const skill = await this.client.playerSkill.findFirst({
  ...
  async apply(
    ...
    await this.client.$transaction(async (tx) => {
      ...
      if (order.status !== "DISPATCHING")
        throw new OrderStateConflictError(orderId, order.status, "APPLY");
      const pub = await tx.dispatchPublication.findFirst({
      ...
      const existing = await tx.application.findFirst({
        where: { tenantId, orderId, playerId },
      });
      if (existing) {
        if (existing.status === "APPLIED" || existing.status === "SHORTLISTED")
          throw new ApplicationConflictError("已报名，请勿重复提交");
        if (existing.status === "SELECTED" || existing.status === "EXPIRED")
          throw new ApplicationConflictError("该报名已失效，不能重新报名");
        await tx.application.update({
          where: { id: existing.id },
          data: { status: "APPLIED", playerNote: note ?? existing.playerNote },
        });
      } else {
        await tx.application.create({
          data: { tenantId, orderId, playerId, playerNote: note ?? null },
        });
      }
      await tx.orderEvent.create({
        data: {
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          aggregateId: orderId,
          eventType: "order.application_applied",
          payload: { orderId, playerId },
        },
      });
    });
  }
  ...
  async hallOrders(tenantId: string): Promise<HallOrderView[]> {
    ...
    for (const pub of pubs) {
      ...
      let productName = "服务";
      if (req?.serviceProductId) {
        const p = await this.client.serviceProduct.findFirst({
          where: { tenantId, id: req.serviceProductId },
          select: { name: true },
        });
  ...
  async shortlist(
    ...
    await this.client.$transaction(async (tx) => {
      const app = await tx.application.findFirst({
        where: { tenantId, orderId, id: applicationId },
      });
      if (!app) throw new DispatchNotFoundError("application", applicationId);
      if (shortlisted && app.status !== "APPLIED")
        throw new ApplicationConflictError("只能对 APPLIED 报名入候选");
      if (
        !shortlisted &&
        app.status !== "APPLIED" &&
        app.status !== "SHORTLISTED"
      )
        throw new ApplicationConflictError("当前状态不可拒绝");
      await tx.application.update({
        where: { id: app.id },
        data: { status: shortlisted ? "SHORTLISTED" : "REJECTED" },
      });
      await tx.orderEvent.create({
        data: {
          tenantId,
          ...
          toStatus: shortlisted ? "SHORTLISTED" : "REJECTED",
          actorType: "tenant_account",
          actorId,
          payload: { applicationId, playerId: app.playerId },
        },
      });
    });
  }
  ...
  async assign(
    ...
    return this.client.$transaction(async (tx) => {
      ...
      const existingAssign = await tx.assignment.findFirst({
        where: { tenantId, orderId },
        select: { id: true },
      });
      ...
      if (app.status !== "APPLIED" && app.status !== "SHORTLISTED")
        throw new ApplicationConflictError("仅 APPLIED/SHORTLISTED 可被选中");
      ...
      await tx.orderEvent.create({
        data: {
          ...
          actorType: "tenant_account",
          actorId,
          payload: { applicationId: app.id, playerId: app.playerId },
        },
      });
      await tx.outboxEvent.create({
        data: {
          ...
          payload: { orderId, orderNo: order.order_no, playerId: app.playerId },
        },
      });
      return {
        id: created.id,
