...
export interface DispatchRepository {
  ...
  apply(tenantId: string, orderId: string, playerId: string, actorId: string, note?: string | null): Promise<void>;
  myApplications(tenantId: string, playerId: string): Promise<ApplicationView[]>;
  applications(tenantId: string, orderId: string): Promise<ApplicationView[]>;
  shortlist(tenantId: string, orderId: string, applicationId: string, shortlisted: boolean, actorId: string): Promise<void>;
  assign(tenantId: string, orderId: string, applicationId: string, actorId: string): Promise<AssignView>;
}
...
export class DispatchService {
  ...
  async shortlist(tenantId: string, orderId: string, applicationId: string, shortlisted: boolean, actorId: string): Promise<void> {
    await this.repository.shortlist(tenantId, orderId, applicationId, shortlisted, actorId);
  }
