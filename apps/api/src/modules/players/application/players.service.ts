...
export interface PlayerRepository {
  list(tenantId: string, opts: { q?: string; status?: "ACTIVE" | "INACTIVE" }): Promise<PlayerView[]>;
  ...
  update(tenantId: string, id: string, input: Partial<PlayerInput>): Promise<PlayerView | null>;
  remove(tenantId: string, id: string): Promise<boolean>;
  gameInTenant(tenantId: string, gameId: string): Promise<boolean>;
  regionInTenant(tenantId: string, regionId: string, gameId: string): Promise<boolean>;
  addSkill(tenantId: string, playerId: string, input: SkillInput): Promise<PlayerSkillView>;
...
function assertPlayerInput(input: Partial<PlayerInput>): void {
  ...
  if (input.mobile !== undefined && input.mobile !== null && typeof input.mobile === "string" && !/^[0-9+\- ]{5,20}$/.test(input.mobile.trim())) {
    throw new InvalidPlayerInputError("mobile 格式非法");
  }
  if (input.intro !== undefined && input.intro !== null && typeof input.intro === "string" && input.intro.length > 500) {
...
export class PlayersService {
  ...
  async update(tenantId: string, id: string, input: Partial<PlayerInput>): Promise<PlayerView> {
    assertPlayerInput(input);
    ...
    if (input.acceptingOrders !== undefined) clean.acceptingOrders = input.acceptingOrders;
  ...
  async addSkill(tenantId: string, playerId: string, input: SkillInput): Promise<PlayerSkillView> {
    ...
    if (input.gameRegionId && !(await this.repository.regionInTenant(tenantId, input.gameRegionId, input.gameId))) {
  ...
  async removeSkill(tenantId: string, playerId: string, skillId: string): Promise<void> {
    const ok = await this.repository.removeSkill(tenantId, playerId, skillId);
    if (!ok) throw new SkillNotFoundError(skillId);
  }

  async addAvailability(tenantId: string, playerId: string, input: AvailabilityInput): Promise<void> {
    if (!(input.endsAt instanceof Date) || !(input.startsAt instanceof Date) || Number.isNaN(input.startsAt.getTime()) || Number.isNaN(input.endsAt.getTime())) {
