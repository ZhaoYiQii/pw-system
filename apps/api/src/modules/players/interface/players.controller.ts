...
export class PlayersController {
  ...
  private mapError(error: unknown): never {
    ...
    if (error instanceof AccountNotPlayerError)
      throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
    if (error instanceof PlayerAccountBoundError)
      throw new HttpException(error.message, HttpStatus.CONFLICT);
    if (
      error instanceof DuplicatePlayerError ||
      error instanceof SkillAlreadyExistsError ||
      error instanceof OverlappingAvailabilityError
    ) {
      throw new HttpException(error.message, HttpStatus.CONFLICT);
    }
    if (
      error instanceof InvalidPlayerInputError ||
      error instanceof GameNotInTenantError ||
      error instanceof RegionNotInTenantError
    ) {
      throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
    }
    throw error;
  ...
  async removeAvailability(
    ...
    try {
      await this.players.removeAvailability(
        tenantIdOf(req),
        playerId,
        availabilityId,
      );
