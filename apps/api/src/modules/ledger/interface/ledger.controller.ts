import { Controller, ForbiddenException, Get, HttpException, HttpStatus, Inject, Param, Post, Req } from "@nestjs/common";
...
export class PlayerFinanceController {
  ...
  async finance(@Req() req: AuthenticatedRequest) {
    ...
    const profile = await this.players.getByAccount(p.tenantId, p.sub).catch(() => {
