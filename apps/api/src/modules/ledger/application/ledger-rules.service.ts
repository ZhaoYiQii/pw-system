import {
  LedgerRulesRepository,
  RuleRow,
} from "../infrastructure/prisma-ledger-rules.repository.js";
...
export class LedgerRulesService {
  ...
  async setStoreCut(tenantId: string, storeCutBp: unknown): Promise<RuleRow> {
    ...
    if (cur.platformFeeBp + bp > 10000)
      throw new Error("platformFeeBp+storeCutBp 不得超过 10000");
    return this.repo.setStoreCut(tenantId, bp);
  }

  async setPlatformFee(
    tenantId: string,
    platformFeeBp: unknown,
  ): Promise<RuleRow> {
    const cur = await this.repo.get(tenantId);
    const bp = this.parseBp(platformFeeBp, "platformFeeBp");
    if (bp + cur.storeCutBp > 10000)
      throw new Error("platformFeeBp+storeCutBp 不得超过 10000");
  ...
  async preview(tenantId: string, amountFen: unknown): Promise<SplitPreview> {
    ...
    if (!/^[1-9][0-9]*$/.test(raw))
      throw new Error("amountFen 必须为正整数字符串（分）");
  ...
  private parseBp(value: unknown, label: string): number {
    ...
    if (!Number.isInteger(bp) || bp < 0 || bp > 10000)
      throw new Error(`${label} 需为 0-10000 的整数（基点）`);
    return bp;
  }
}
