import { Controller, ForbiddenException, Get, HttpException, HttpStatus, Inject, Req } from "@nestjs/common";
  ...
  async list(@Req() req: AuthenticatedRequest) {
    ...
    return withTenantContext(this.client, tenantId, async (tx: DbTransaction) => {
      ...
      if (role !== undefined && STAFF_ROLES.has(role)) {
        ...
        const profile = await this.customers.getByAccount(tenantId, req.principal?.sub ?? "").catch(() => null);
        ...
        const player = await this.players.getByAccount(tenantId, req.principal?.sub ?? "").catch(() => null);
        ...
        const ids = Array.from(new Set([...assignments, ...applications].map((r) => r.orderId)));
        if (ids.length === 0) return { data: [] };
        where.recipientType = "order";
        where.recipientId = { in: ids };
      } else {
