...
export class CustomerSelfController {
  ...
  async me(@Req() req: AuthenticatedRequest) {
    ...
    try {
      return {
        data: await this.customers.getByAccount(ctx.tenantId, ctx.accountId),
      };
    } catch (error) {
  ...
  async myOrders(@Req() req: AuthenticatedRequest) {
    const ctx = this.requireCustomer(req);
    try {
      const profile = await this.customers.getByAccount(
        ctx.tenantId,
        ctx.accountId,
      );
      return {
        data: await this.orders.listByCustomer(ctx.tenantId, profile.id),
      };
    } catch (error) {
      if (error instanceof CustomerNotFoundError) {
        throw new HttpException("尚未绑定客户档案", HttpStatus.NOT_FOUND);
  ...
  async createOrder(
    ...
    try {
      ...
      const created = await this.orders.create(ctx.tenantId, ctx.accountId, {
        ...
        requirement: ((body.requirement as
