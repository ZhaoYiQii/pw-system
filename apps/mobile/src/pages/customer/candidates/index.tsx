...
interface OrderRow { id: string; orderNo: string; status: string; customerName: string }
interface Candidate { id: string; playerName: string; status: string; playerNote: string | null }
...
export default function CandidatesPage() {
  ...
  const loadOrders = async (t: string) => {
    try {
      const list = (await api("/api/v1/tenant/customer/orders", t)) as OrderRow[];
      setOrders(list);
    } catch (error) {
  ...
  const open = async (orderId: string) => {
    ...
    setMsg(null);
    try {
      const list = (await api(`/api/v1/tenant/customer/orders/${orderId}/applications`, token)) as Candidate[];
      setCands(list.filter((c) => c.status === "APPLIED" || c.status === "SHORTLISTED"));
    } catch (error) {
      setMsg(error instanceof Error ? error.message : String(error));
    }
  };
  ...
  const select = async (orderId: string, appId: string) => {
    ...
    setMsg(null);
    try {
      await api(`/api/v1/tenant/customer/orders/${orderId}/assignment`, token, { method: "POST", body: JSON.stringify({ applicationId: appId }) });
    ...
    <View className="page">
      ...
      {token === null ? (
        <View className="card">
          ...
          <Input className="input" value={username} onInput={(e) => setUsername(e.detail.value)} placeholder="customer" />
          <Text className="label">密码</Text>
          <Input className="input" password value={password} onInput={(e) => setPassword(e.detail.value)} placeholder="••••" />
          <Button className="btn" disabled={busy} onClick={() => void login()}>登录并查看我的订单</Button>
          {msg ? <Text className="err">{msg}</Text> : null}
        </View>
      ) : (
        <>
          {msg ? <Text className="err">{msg}</Text> : null}
          {orders.length === 0 ? <Text className="muted">暂无订单（可由商家端为你创建并确认/发布）</Text> : null}
          {orders.map((o) => (
            <View key={o.id} className="card">
              <View className="row">
                <Text className="strong">{o.orderNo}</Text>
                <Text className="badge open">{o.status === "DISPATCHING" ? "正在选人" : o.status}</Text>
              </View>
              {o.status === "DISPATCHING" ? (
                <>
                  {openOrder === o.id ? (
                    cands.length === 0 ? <Text className="muted">暂无候选</Text> : null
                  ) : null}
                  {openOrder === o.id
                    ? cands.map((c) => (
                        <View key={c.id} className="row">
                          <View>
                            ...
                            {c.playerNote ? <Text className="muted">（{c.playerNote}）</Text> : null}
                          </View>
                          <Button size="mini" disabled={busy} onClick={() => void select(o.id, c.id)}>选 TA</Button>
                        </View>
                      ))
                    : null}
                  {openOrder !== o.id ? (
                    <Button className="btn" onClick={() => void open(o.id)}>查看候选并选人</Button>
                  ) : null}
                </>
              ) : null}
            </View>
