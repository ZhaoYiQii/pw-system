...
对照 `apps/admin-web/app/_lib/merchant-console` 与 `apps/api/src/modules` 复核第 2 节清单：

- 已落地（新控制台页面 + 真实接口）：门店概览/监控台、AI 需求助手、场次台账与详情时间线、证据查看、时长/金额调整复核、结算批次列表与详情（复核/批准/登记支付/冲正）、财务风险、客诉/争议详情、审计日志筛选与导出、通知中心、客户与陪玩详情、游戏模板/服务产品/定价规则、员工与套餐卡片、五态与 403 处理。
- 未落地（导航以 preview/planned 登记，不调用业务接口）：抢单大厅、评价管理、排班管理、标签分组、关系管理、员工管理、价格方案、分成方案、附加服务、限购方案、技能证书、店铺装修、优惠活动、邀请奖励、会员成长、权益礼包、用户背包、互动玩法、提现管理、客户关怀、流失预警、私单风险、异常监测。
- 结论：原第一优先级（场次与证据）已完成；剩余缺口集中在营销会员、提现与数据风控域，需后端模型先行，详见 `docs/DEVELOPMENT_BACKLOG.md` 第一节。
