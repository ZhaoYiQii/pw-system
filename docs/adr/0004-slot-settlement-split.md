...
- 状态：**已批准（用户 2026-09-21，4 个待批准项全部按本 ADR 的建议执行）**
- 日期：2026-09-21
...
- 2026-09-21 用户批准：**「4 条都按建议」**——① 审批通过即分账（`amountFen` = 陪玩实收，`detailJson` 保留 `grossFen/platformFeeFen/storeCutFen`）；② 暂不引入 `LedgerEntry`；③ 三处默认值置 0 + 幂等迁移补齐既有租户的费率行；④ 历史数据不回写。
- 同日授权：由 Codex 按本 ADR 实现（单独切片、先红后绿、全套门禁 + E2E 复跑 + 走查复验）。
