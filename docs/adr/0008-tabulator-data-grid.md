...
- 状态：**已批准**（用户 2026-09-23「ADR 批准 + 稿子照这样 + 装」）。UI 设计稿见 `design-demos/finance-data-grid.html` / `.png`。
- 日期：2026-09-23
...
- **2026-09-23 用户正式批准**：「ADR 批准 + 稿子照这样 + 装」——同时授权安装 `tabulator-tables`（MIT）。
- 批准范围：本 ADR 的决定 1–5；S5-1（表格壳 + 支付台账样板）开始实施。行内编辑（S5-2）与粘贴导入（S5-3）仍按片逐个验收。
