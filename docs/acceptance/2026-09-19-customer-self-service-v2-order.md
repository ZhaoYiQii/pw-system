...
- 归因：Task 3 的「幂等 operation 按入口区分」改动后，该 spec 走的路径没带 `operation`；本回合只改了客户只读入口与 H5，未触碰幂等链路。
- 处置建议：另立小切片修 spec 或补 `operation` 默认分支，并把它纳入 Task 5 门禁清单；本回合未修，因此**不能声称租户隔离套件全绿**。
