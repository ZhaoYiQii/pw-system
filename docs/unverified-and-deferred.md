...
- 证据基准：`master` @ `97bcc4c`（2026-09-10）+ 工作树未提交改动；历史切片明细回查 git log `cce1ca0..3692450`。
...
| 项                                                         | 状态日期   | 解除条件                                                            |
| ---------------------------------------------------------- | ---------- | ------------------------------------------------------------------- |
| H1 Linux 容器实机验证（Dockerfile 与 prod compose 已就绪） | 2026-09-11 | 提供 Linux 主机/服务器后实机跑通迁移、seed 与四端启动               |
...
- 覆盖率质量债：`test:coverage` 已合并单元+集成，branch 63.7%（目标 80%；critical-domain 已达 100/100/100/100），2026-09-08 决定延后。
