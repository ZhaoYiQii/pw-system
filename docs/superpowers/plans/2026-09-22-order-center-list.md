...
3. **写动作边界**：本片只接行级「发布」「释放」，批量只接「批量通过（逐条串行 + 失败汇总）」与「导出 CSV（按选中）」。
   ...
   **落定（2026-09-22，用户选 1）**：批量释放**不做**。原因是释放接口按档位（`POST /slots/{slotId}/release`）且列表行没有 `slotId`，批量要走「每单先取详情 → 逐个释放」，属于有副作用的重写操作，风险与「批量通过」不同级。释放只在订单详情页按单执行；如后续要在列表批量释放，单独开一片并带确认弹窗、逐条失败汇总与限流。

   **本片实际落地的批量条**：「批量通过」（逐条串行 + 失败汇总）与「导出 CSV（按选中）」。
