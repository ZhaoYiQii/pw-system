...
describe("tenant-config v1 schema", () => {
  ...
  it("非法颜色被拒绝", () => {
    const bad = { ...DEFAULT_TENANT_CONFIG, brand: { ...DEFAULT_TENANT_CONFIG.brand, primaryColor: "red" } };
    expect(safeParseTenantConfig(bad).success).toBe(false);
  });

  it("任意 HTML/CSS/JS 类字段与未知字段被拒绝（strict）", () => {
    const withUnknown = { ...DEFAULT_TENANT_CONFIG, customCssUrl: "https://evil.example/x.css" };
  ...
  it("borderRadius 越界被拒绝", () => {
    const bad = { ...DEFAULT_TENANT_CONFIG, brand: { ...DEFAULT_TENANT_CONFIG.brand, borderRadius: 99 } };
    expect(safeParseTenantConfig(bad).success).toBe(false);
  });
  ...
  it("分层合并以 override 为准并保持合法", () => {
    const override = parseTenantConfig({
      ...DEFAULT_TENANT_CONFIG,
      brand: { ...DEFAULT_TENANT_CONFIG.brand, primaryColor: "#111111", logoText: "Demo" }
    });
