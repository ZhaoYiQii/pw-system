...
describe("Slice 6 dispatch (publish/apply/shortlist/assign 并发与冲突)", () => {
  ...
  beforeAll(async () => {
    ...
    const customer = await client.customerProfile.create({
      data: { tenantId, name: "派单客" },
    });
    const game = await client.game.create({
      data: { tenantId, name: "绝地求生" },
    });
    const product = await client.serviceProduct.create({
      data: { tenantId, gameId: game.id, name: "吃鸡陪玩" },
    });
    ...
    await client.pricingRule.create({
      data: {
        ...
        priceFen: BigInt(3000),
        playerCostFen: BigInt(1500),
      },
    });
    ...
    async function makePlayer(username: string, name: string) {
      ...
      const profile = await client.playerProfile.create({
        data: { tenantId, name, tenantAccountId: acc.id },
      });
      await client.playerSkill.create({
        data: { tenantId, playerId: profile.id, gameId: game.id },
      });
      return { acc, profile };
    }
  ...
  async function makeConfirmedOrder() {
    const customers = await client.customerProfile.findFirst({
      where: { tenantId },
    });
    ...
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${order.id}/confirm`)
      .expect(201);
    return order.id;
  }
  ...
  it("发布→双人报名→短名单→并发选人仅一人成功且其余过期", async () => {
    ...
    await req(ownerToken)
      ...
      .expect(201);
    const hall = (
      await req(p1Token).get("/api/v1/tenant/player/order-hall").expect(200)
    ).body.data as Array<{ id: string; unitPriceFen: string }>;
    ...
    const [a, b] = await Promise.all([
      req(ownerToken).post(`/api/v1/tenant/orders/${orderId}/assignment`, {
    ...
    expect(
      await client.assignment.count({ where: { tenantId, orderId } }),
    ).toBe(1);
    expect(
      (await client.order.findFirst({ where: { tenantId, id: orderId } }))
        ?.status,
    ).toBe("ASSIGNED");
    expect(
      (
        await client.dispatchPublication.findFirst({
          where: { tenantId, orderId },
        })
      )?.status,
    ).toBe("CLOSED");
    const appRows = await client.application.findMany({
      where: { tenantId, orderId },
    });
  ...
  it("重复报名 409；时间冲突报名 400", async () => {
    ...
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${o1}/publish`)
      .expect(201);
    await req(p1Token)
      .post(`/api/v1/tenant/player/orders/${o1}/applications`)
      .expect(201);
    await req(p1Token)
      .post(`/api/v1/tenant/player/orders/${o1}/applications`)
      .expect(409);

    const p1row = await client.playerProfile.findFirst({
      where: { tenantId, name: "陪玩一" },
    });
    await client.playerAvailability.create({
      data: {
        tenantId,
        playerId: p1row?.id as string,
        startsAt: new Date("2031-01-01T10:00:00Z"),
        endsAt: new Date("2031-01-01T14:00:00Z"),
      },
    });
    const customers = await client.customerProfile.findFirst({
      where: { tenantId },
    ...
    const res = await req(ownerToken)
      .post("/api/v1/tenant/orders", {
        ...
        requirement: {
          ...
          serviceProductId: productId,
          durationSeconds: 3600,
          desiredStartAt: "2031-01-01T12:00:00Z",
        },
      })
      .expect(201);
    const o2 = (res.body.data as { id: string }).id;
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${o2}/confirm`)
      .expect(201);
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${o2}/publish`)
      .expect(201);
    await req(p1Token)
      .post(`/api/v1/tenant/player/orders/${o2}/applications`)
      .expect(400);
  });

  it("老板（客户本人）可看候选并自助选人；非本人订单 403", async () => {
    // 绑定 customer 账号到订单所属客户档案
    const customerProfile = (await client.customerProfile.findFirst({
      where: { tenantId },
    })) as { id: string };
    const hash = await hashPassword(PW);
    const acc = await client.tenantAccount.create({
      data: { tenantId, username: `boss_c_${suffix}`, passwordHash: hash },
    });
    await client.tenantAccountRole.create({
    ...
    const candidates = (
      await req(cToken)
        .get(`/api/v1/tenant/customer/orders/${orderId}/applications`)
        .expect(200)
    ).body.data as Array<{ id: string }>;
