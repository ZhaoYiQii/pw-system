...
import { drainOutbox, requeueFailedOutboxEvent } from "../../apps/api/src/modules/notifications/outbox.relay.js";
...
describe("C3 outbox relay: tenant scope / lease recovery (重放不重复)", () => {
  ...
  beforeAll(async () => {
    ...
    tenantId = t.id;
    const staleT = await client.tenant.create({ data: { code: `oxs_${suffix}`, name: "outbox残留店" } });
  ...
  async function seedOwnPending(): Promise<string[]> {
    const created = await client.outboxEvent.createMany({
      data: [
        { tenantId, aggregateType: "order", aggregateId: `own-1-${suffix}`, eventType: "order.confirmed", payload: { orderNo: "A1" } },
        { tenantId, aggregateType: "order", aggregateId: `own-2-${suffix}`, eventType: "order.accounted", payload: { orderNo: "A2" } }
      ]
    });
  ...
  afterAll(async () => {
    if (client) {
      ...
      await client.notificationDelivery.deleteMany({ where: { tenantId: staleTenantId } });
      await client.outboxEvent.deleteMany({ where: { tenantId } });
      await client.outboxEvent.deleteMany({ where: { tenantId: staleTenantId } });
  ...
  it("PROCESSING 超过租约时间会被回收并重新消费（不永久卡死）", async () => {
    ...
    expect(n).toBe(1);
    const after = await client.outboxEvent.findUnique({ where: { id: stuck.id }, select: { status: true } });
    expect(after?.status).toBe("PROCESSED");
    const notif = await client.notificationDelivery.findFirst({ where: { tenantId, outboxEventId: stuck.id } });
  ...
  it("FAILED 事件可人工重放：reset 后重新 drain 成功", async () => {
    ...
    const n = await drainOutbox(client, 20, tenantId);
    expect(n).toBe(1);
    const after = await client.outboxEvent.findUnique({ where: { id: failed.id }, select: { status: true, attempts: true } });
    expect(after?.status).toBe("PROCESSED");
