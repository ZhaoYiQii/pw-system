...
describe("Slice 9 dispute freeze & audit", () => {
  ...
  it("开放争议 earning 不能入批/不能支付；处理后审计可查", async () => {
    const orderA = (
      await client.earning.findFirst({ where: { tenantId, id: eIds[0] } })
    )?.orderId as string;
    const d = (
      await req(ownerToken)
        .post(`/api/v1/tenant/orders/${orderA}/disputes`, {
          reason: "服务中途消失",
          earningId: eIds[0],
        })
        .expect(201)
    ...
    await req(ownerToken)
      .post(`/api/v1/tenant/settlements/${b1.id}/items`, {
        earningIds: [eIds[0] as string],
      })
      .expect(409);

    // 另一 earning 入批、复核、批准，但随后开争议 → 支付被冻结
    const orderB = (
      await client.earning.findFirst({ where: { tenantId, id: eIds[1] } })
    )?.orderId as string;
    const b2 = (
      await req(ownerToken).post("/api/v1/tenant/settlements").expect(201)
    ).body.data as { id: string };
    await req(ownerToken)
      .post(`/api/v1/tenant/settlements/${b2.id}/items`, {
        earningIds: [eIds[1] as string],
      })
      .expect(201);
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${orderB}/disputes`, {
        reason: "场次不符",
        earningId: eIds[1],
      })
      .expect(201);
    await req(ownerToken)
      .post(`/api/v1/tenant/settlements/${b2.id}/review`)
      .expect(201);
    ...
    // 处理争议后可支付
    await req(ownerToken)
      .post(`/api/v1/tenant/disputes/${d.id}/resolve`, {
        resolution: "已退款并结案",
      })
      .expect(201);
    await req(ownerToken)
    ...
    for (const open of allOpen) {
      await req(ownerToken)
        .post(`/api/v1/tenant/disputes/${open.id}/resolve`, {
          resolution: "调解完成",
        })
        .expect(201);
    }
