...
describe("Slice 4 catalog/customers/players HTTP (权限/价格边界/跨租户/重叠)", () => {
  ...
  beforeAll(async () => {
    ...
    const t1 = await client.tenant.create({
      data: { code: tenantCode, name: "S4店" },
    });
    const t2 = await client.tenant.create({
      data: { code: otherTenantCode, name: "S4隔壁店" },
    });
    const owner = await client.tenantAccount.create({
      data: { tenantId: t1.id, username: "owner", passwordHash: hash },
    });
    const service = await client.tenantAccount.create({
      data: { tenantId: t1.id, username: "service", passwordHash: hash },
    });
    ...
    await client.tenantAccountRole.create({
      data: {
        tenantId: t1.id,
        tenantAccountId: service.id,
        role: "CUSTOMER_SERVICE",
      },
    });
    const otherGame = await client.game.create({
      data: { tenantId: t2.id, name: "隔壁游戏" },
    });
    ...
    if (client) {
      for (const code of [tenantCode, otherTenantCode]) {
        ...
        for (const t of tenants) {
          ...
          await client.playerAvailability.deleteMany({
            where: { tenantId: t.id },
          });
  ...
  function req(token: string) {
    ...
    return {
      ...
      del: (url: string) =>
        request(app.getHttpServer()).delete(url).set(headers),
    };
  }
  ...
  it("catalog：owner 建游戏/区服/产品/价格，价格读取为十进制字符串分", async () => {
    const gameRes = await req(ownerToken)
      .post("/api/v1/tenant/catalog/games")
      .send({ name: "王者荣耀" })
      .expect(201);
    ...
    const ruleRes = await req(ownerToken)
      ...
      .expect(201);
    expect(
      (ruleRes.body.data as { priceFen: string; playerCostFen: string })
        .priceFen,
    ).toBe("1500");
    expect((ruleRes.body.data as { playerCostFen: string }).playerCostFen).toBe(
      "800",
    );
    const list = await req(ownerToken)
      .get(`/api/v1/tenant/catalog/products/${productId}/pricing`)
      .expect(200);
    expect((list.body.data as Array<{ priceFen: string }>)[0]?.priceFen).toBe(
      "1500",
    );
  });
  ...
  it("跨租户技能关联被拒（引用他店游戏 → 400）", async () => {
    const playerRes = await req(ownerToken)
      .post("/api/v1/tenant/players")
      .send({ name: "阿伟" })
      .expect(201);
  ...
  it("重叠不可用时间被拒，非重叠与开关正常", async () => {
    const players = await req(ownerToken)
      .get("/api/v1/tenant/players")
      .expect(200);
    const playerId = (players.body.data as Array<{ id: string }>)[0]
      ?.id as string;
    await req(ownerToken)
    ...
    const detail = await req(ownerToken)
      .get(`/api/v1/tenant/players/${playerId}`)
      .expect(200);
    expect(
      (detail.body.data as { availability: unknown[] }).availability,
    ).toHaveLength(2);
    await req(ownerToken)
      .patch(`/api/v1/tenant/players/${playerId}`)
      .send({ acceptingOrders: false })
      .expect(200);
    const after = await req(ownerToken)
      .get(`/api/v1/tenant/players/${playerId}`)
      .expect(200);
    expect(
      (after.body.data as { acceptingOrders: boolean }).acceptingOrders,
    ).toBe(false);
  });
  it("绑定 PLAYER 账号后陪玩自助 me/接单/排期可用；非 PLAYER 403", async () => {
    ...
    const playerId = (playerRes.body.data as { id: string }).id;
    await req(ownerToken)
      .post(`/api/v1/tenant/players/${playerId}/account`)
      .send({ accountId: acc.id })
      .expect(201);

    // 绑定到非 PLAYER 账号被拒
    const serviceAcc = await client.tenantAccount.findFirst({
      where: { tenantId, username: "service" },
    });
    if (serviceAcc) {
      await req(ownerToken)
        .post(`/api/v1/tenant/players/${playerId}/account`)
        .send({ accountId: serviceAcc.id })
        .expect(400);
    }
    ...
    const playerLogin = await request(app.getHttpServer())
      ...
      .expect(201);
    const pToken = (playerLogin.body as { data: Data }).data
      .accessToken as string;

    const me = await req(pToken).get("/api/v1/tenant/player/me").expect(200);
    expect((me.body.data as { name: string }).name).toBe("自助陪玩");
    expect((me.body.data as { acceptingOrders: boolean }).acceptingOrders).toBe(
      true,
    );

    await req(pToken)
      .patch("/api/v1/tenant/player/me")
      .send({ acceptingOrders: false })
      .expect(200);
    const me2 = await req(pToken).get("/api/v1/tenant/player/me").expect(200);
    expect(
      (me2.body.data as { acceptingOrders: boolean }).acceptingOrders,
    ).toBe(false);
    ...
    const av = (me3.body.data as { availability: Array<{ id: string }> })
      .availability;
    expect(av).toHaveLength(1);
    await req(pToken)
      .del(`/api/v1/tenant/player/availability/${av[0]?.id as string}`)
      .expect(200);
