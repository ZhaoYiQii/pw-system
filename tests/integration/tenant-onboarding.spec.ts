...
describe("Slice 11 onboarding (平台一键开通/套餐/停用不可登录)", () => {
  ...
  beforeAll(async () => {
    ...
    await client.platformAccount.create({ data: { username: `pf11_${suffix}`, passwordHash: hash, role: "PLATFORM_SUPER_ADMIN" } });
    const moduleRef = await Test.createTestingModule({ imports: [AppModule] }).compile();
    app = moduleRef.createNestApplication();
    await app.init();
    const login = await request(app.getHttpServer()).post("/api/v1/auth/login").send({ kind: "platform", username: `pf11_${suffix}`, password: PW }).expect(201);
  ...
  afterAll(async () => {
    if (client) {
      ...
      if (t) {
        ...
        await client.tenantConfigVersion.deleteMany({ where: { tenantId: tid } });
      ...
      await client.platformAccount.deleteMany({ where: { username: `pf11_${suffix}` } });
  ...
  it("配置不完整不能开通；完整可开通→owner 可登录→套餐指派后 addon 开启", async () => {
    await req(platformToken).post("/api/v1/platform/onboarding/tenants", {
      code: tenantCode, name: "开通店", host: `${tenantCode}.example.com`, ownerUsername: "boss", ownerPassword: "x"
    }).expect(400);

    const onboard = (await req(platformToken).post("/api/v1/platform/onboarding/tenants", {
      code: tenantCode, name: "开通店", host: `${tenantCode}.example.com`, ownerUsername: "boss", ownerPassword: "Onboard-123",
      brandPrimary: "#16a34a", storeCutBp: 2000, packageCode: "BASIC"
    }).expect(201)).body.data as { tenantId: string };
    tenantId = onboard.tenantId;

    const ownerLogin = await request(app.getHttpServer()).post("/api/v1/auth/login").send({ kind: "tenant", tenantCode, username: "boss", password: "Onboard-123" }).expect(201);
    const ownerToken = (ownerLogin.body as { data: Data }).data.accessToken as string;
    ...
    await req(platformToken).post(`/api/v1/platform/tenants/${tenantId}/package`, { packageCode: "PRO" }).expect(201);
    const feats = (await req(ownerToken).get("/api/v1/tenant/features").expect(200)).body.data as Array<{ featureKey: string; enabled: boolean }>;
    expect(feats.find((f) => f.featureKey === "addon.customer_self_service")?.enabled).toBe(true);
  });
});