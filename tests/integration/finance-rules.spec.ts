...
describe("Slice 8 finance rules (默认 3%/20%，可后台调整，split preview)", () => {
  ...
  beforeAll(async () => {
    ...
    const pa = await client.platformAccount.create({
      data: {
        username: `pf_${suffix}`,
        passwordHash: hash,
    ...
    const pRes = await request(app.getHttpServer())
      .post("/api/v1/auth/login")
      .send({ kind: "platform", username: `pf_${suffix}`, password: PW })
      .expect(201);
  ...
  it("默认 3%/20%；门店可改门店抽成；preview 分配守恒", async () => {
    ...
    const preview = (
      await req(ownerToken)
        .post("/api/v1/tenant/finance-rules/split-preview", {
          amountFen: "10000",
        })
        .expect(201)
    ).body.data as {
    ...
    expect(
      BigInt(preview.platformFeeFen) +
        BigInt(preview.storeCutFen) +
    ...
    const set = await request(app.getHttpServer())
      .post("/api/v1/tenant/finance-rules/store-cut")
      .set("authorization", `Bearer ${ownerToken}`)
      .send({ storeCutBp: 1500 })
      .expect(201);
  ...
  it("平台可调平台费率（跨租户）", async () => {
    const patched = await request(app.getHttpServer())
      ...
      .expect(200);
    const data = patched.body.data as {
