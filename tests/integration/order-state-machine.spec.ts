...
describe("B1 order state machine (完整迁移链，无跳状态)", () => {
  ...
  beforeAll(async () => {
    ...
    const t = await client.tenant.create({
      data: { code: `sm_${suffix}`, name: "状态机店" },
    });
    tenantId = t.id;
    ...
    const pAcc = await client.tenantAccount.create({
      data: { tenantId, username: "player", passwordHash: hash },
    });
    await client.tenantAccountRole.create({
      data: { tenantId, tenantAccountId: pAcc.id, role: "PLAYER" },
    });
    const player = await client.playerProfile.create({
      data: { tenantId, name: "状态机陪玩", tenantAccountId: pAcc.id },
    });
    const custAcc = await client.tenantAccount.create({
      data: { tenantId, username: "customer", passwordHash: hash },
    });
    await client.tenantAccountRole.create({
      data: { tenantId, tenantAccountId: custAcc.id, role: "CUSTOMER" },
    });
    const cust = await client.customerProfile.create({
      data: { tenantId, name: "状态机客", tenantAccountId: custAcc.id },
    });
    const game = await client.game.create({ data: { tenantId, name: "游戏" } });
    const product = await client.serviceProduct.create({
      data: { tenantId, gameId: game.id, name: "产品" },
    });
    await client.pricingRule.create({
      data: {
        ...
        priceFen: BigInt(10000),
        playerCostFen: BigInt(0),
      },
    });
  ...
  async function makeOrder(status?: string): Promise<{ id: string }> {
    ...
    const products = (
      await req(ownerToken)
        .get(`/api/v1/tenant/catalog/products?gameId=${games[0]?.id}`)
        .expect(200)
    ).body.data as Array<{ id: string }>;
    if (status) {
      const row = await client.order.create({
        data: {
          tenantId,
          orderNo: `smc_${Date.now().toString(36)}`,
          customerProfileId: customers[0]?.id as string,
  ...
  it("完整迁移链：ASSIGNED→READY→IN_PROGRESS→PENDING_CONFIRMATION→COMPLETED", async () => {
    ...
    await req(ownerToken)
      ...
      .expect(201);
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${order.id}/publish`)
      .expect(201);
    await req(playerToken)
      .post(`/api/v1/tenant/player/orders/${order.id}/applications`)
      .expect(201);
    const apps = (
      await req(ownerToken)
        .get(`/api/v1/tenant/orders/${order.id}/applications`)
        .expect(200)
    ).body.data as Array<{ id: string }>;
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${order.id}/assignment`, {
    ...
    await req(playerToken)
      .post(`/api/v1/tenant/orders/${order.id}/session/end`)
      .expect(201);
    view = (
      await req(ownerToken).get(`/api/v1/tenant/orders/${order.id}`).expect(200)
    ).body.data as { status: string; timeline: Array<{ eventType: string }> };
    ...
    expect(
      view.timeline.filter((e) => e.eventType === "ORDER_SESSION_ENDED"),
    ).toHaveLength(1);

    const accounted = (
      await req(customerToken)
        .post(`/api/v1/tenant/customer/orders/${order.id}/complete`)
        .expect(201)
    ...
    const accountedEvent = view.timeline.find(
      (e) => e.eventType === "ORDER_ACCOUNTED",
    );
    expect(accountedEvent?.fromStatus).toBe("PENDING_CONFIRMATION");
    expect(accountedEvent?.toStatus).toBe("COMPLETED");
    const sessionView = (
      await req(ownerToken)
        .get(`/api/v1/tenant/orders/${order.id}/session`)
        .expect(200)
    ).body.data as { status: string };
  ...
  it("DISPATCHING 订单可直接取消（状态机可取消集合）", async () => {
    ...
    const cancelled = (
      await req(ownerToken)
        .post(`/api/v1/tenant/orders/${order.id}/cancel`, {
          reason: "临时取消",
        })
        .expect(201)
    ).body.data as { status: string };
