...
import { ClientSuppliedTenantIdError, DuplicateTenantCodeError, InvalidTenantCodeError, TenantNotFoundError } from "../../apps/api/src/modules/tenancy/domain/errors.js";
...
describe("tenancy service (application + repository)", () => {
  ...
  it("创建租户（含主域名）后可按域名解析为 ACTIVE", async () => {
    ...
    const created = await service.createTenant({ code, name: "服务测试店", primaryHost: host });
  ...
  it("重复 code 抛 DuplicateTenantCodeError", async () => {
    ...
    await expect(service.createTenant({ code, name: "重复" })).rejects.toBeInstanceOf(DuplicateTenantCodeError);
  });

  it("非法 code 抛 InvalidTenantCodeError", async () => {
    await expect(service.createTenant({ code: "BAD CODE!", name: "x" })).rejects.toBeInstanceOf(InvalidTenantCodeError);
  });
  ...
  it("停用后状态为 INACTIVE（H5 据此显示不可用）", async () => {
    ...
    const created = await service.createTenant({ code, name: "待停用", primaryHost: host });
  ...
  it("拒绝客户端自报 tenantId（主规格 8.1）", () => {
    expect(() => service.assertNoClientTenantId({ code: "ok", name: "x" })).not.toThrow();
    expect(() => service.assertNoClientTenantId({ tenantId: "00000000-0000-4000-8000-000000000000" })).toThrow(
