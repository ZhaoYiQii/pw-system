...
describe("Slice 10 AI (deterministic/不可用标记/合法候选)", () => {
  ...
  beforeAll(async () => {
    ...
    await client.tenantEntitlement.createMany({
      data: [
        ...
        {
          tenantId,
          featureKey: "addon.ai_match_recommendation",
          enabled: true,
          source: "test",
        },
      ],
    ...
    const cust = await client.customerProfile.create({
      data: { tenantId, name: "AI客" },
    });
    const game = await client.game.create({
      data: { tenantId, name: "无畏契约" },
    });
    const product = await client.serviceProduct.create({
      data: { tenantId, gameId: game.id, name: "双排1h" },
    });
    const p1 = await client.playerProfile.create({
      data: { tenantId, name: "有技能" },
    });
    const p2 = await client.playerProfile.create({
      data: { tenantId, name: "无技能" },
    });
    await client.playerSkill.create({
      data: { tenantId, playerId: p1.id, gameId: game.id },
    });
    ...
    orderId = order.id;
    await client.orderRequirement.create({
      data: {
        tenantId,
        orderId: order.id,
        description: "双排",
  ...
  afterAll(async () => {
    if (client) {
      ...
      await client.orderEvent
        .deleteMany({ where: { tenantId: { in: [tenantId] } } })
        .catch(() => undefined);
  ...
  it("capabilities 明确不可用；缺失检查 NEEDS_REVIEW；候选仅合法集合", async () => {
    const caps = (
      await req(ownerToken).get("/api/v1/tenant/ai/capabilities").expect(200)
    ).body.data as { supported: boolean; reason: string };
    ...
    const rec = (
      await req(ownerToken)
        ...
        .expect(200)
    ).body.data as { candidates: Array<{ name: string }> };
    expect(rec.candidates.map((x) => x.name)).toEqual(["有技能"]);
    expect(
      await client.aiRun.count({ where: { tenantId } }),
