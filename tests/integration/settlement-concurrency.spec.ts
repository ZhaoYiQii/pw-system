...
describe("Slice 8 settlement (批次状态机/并发唯一/职责分离)", () => {
  ...
  beforeAll(async () => {
    ...
    const t = await client.tenant.create({ data: { code: `st_${suffix}`, name: "结算店" } });
    tenantId = t.id;
    ...
    const player = await client.playerProfile.create({ data: { tenantId, name: "结算玩" } });
    for (let i = 1; i <= 2; i += 1) {
      const cust = await client.customerProfile.create({ data: { tenantId, name: `客${i}` } });
      const order = await client.order.create({ data: { tenantId, orderNo: `so${i}_${suffix}`, customerProfileId: cust.id } });
      const e = await client.earning.create({ data: { tenantId, orderId: order.id, playerId: player.id, amountFen: BigInt(7700) } });
  ...
  it("DRAFT 批次加 earning；重复入其他批次 409；职责分离：发起人不能批准；PAID 后不可改", async () => {
    ...
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b1.id}/items`, { earningIds }).expect(201);

    // 同一 earning 进第二个批次被拒
    const b2 = (await req(ownerToken).post("/api/v1/tenant/settlements").expect(201)).body.data as { id: string };
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b2.id}/items`, { earningIds: [earningIds[0] as string] }).expect(409);

    // 状态流转：review -> 发起人不能 approve -> finance approve -> pay
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b1.id}/review`).expect(201);
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b1.id}/approve`).expect(409);
    await req(financeToken).post(`/api/v1/tenant/settlements/${b1.id}/approve`).expect(201);
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b1.id}/pay`).expect(201);

    const paid = await client.earning.findMany({ where: { tenantId, id: { in: earningIds } } });
    expect(paid.every((e) => e.status === "PAID")).toBe(true);
    // PAID 后不可再添加/冲正
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b1.id}/items`, { earningIds: [earningIds[1] as string] }).expect(409);
  ...
  it("VOID 可让 DRAFT/REVIEWED 批次退回 earning 为 PENDING", async () => {
    ...
    const e3 = await client.earning.create({ data: { tenantId, orderId: order.id, playerId: player?.id as string, amountFen: BigInt(7700) } });
    const b3 = (await req(ownerToken).post("/api/v1/tenant/settlements").expect(201)).body.data as { id: string };
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b3.id}/items`, { earningIds: [e3.id] }).expect(201);
    await req(ownerToken).post(`/api/v1/tenant/settlements/${b3.id}/void`).expect(201);
    const after = await client.earning.findFirst({ where: { tenantId, id: e3.id } });
