...
describe("Slice 5 orders (DRAFT->CONFIRMED / 快照稳定 / 幂等 / Outbox)", () => {
  ...
  beforeAll(async () => {
    ...
    const owner = await client.tenantAccount.create({
      data: { tenantId: t.id, username: "boss", passwordHash: hash },
    });
    await client.tenantAccountRole.create({
      data: { tenantId: t.id, tenantAccountId: owner.id, role: "TENANT_OWNER" },
    });
    await client.customerProfile.create({
      data: { tenantId: t.id, name: "下单客" },
    });
    const game = await client.game.create({
      data: { tenantId: t.id, name: "英雄联盟" },
    });
    const product = await client.serviceProduct.create({
      data: { tenantId: t.id, gameId: game.id, name: "LOL 陪玩1小时" },
    });
    await client.pricingRule.create({
      data: {
        ...
        priceFen: BigInt(2000),
        playerCostFen: BigInt(1000),
      },
    });
    ...
    if (client) {
      ...
      for (const t of tenants) {
        ...
        for (const o of orders) {
          ...
          await client.orderRequirement.deleteMany({
            where: { orderId: o.id },
          });
        }
  ...
  async function setupProduct(priceFen: number) {
    ...
    await req(ownerToken)
      ...
      .send({ durationSeconds: 3600, priceFen: String(priceFen) })
      .expect(201);
    return { gameId, productId };
  }

  it("创建草稿、缺需求/缺产品被拒、确认生成快照并进入 CONFIRMED", async () => {
    const customers = await req(ownerToken)
      .get("/api/v1/tenant/customers")
      .expect(200);
    const customerId = (customers.body.data as Array<{ id: string }>)[0]
      ?.id as string;
    ...
    await req(ownerToken)
      ...
      .send({
        ...
        requirement: { serviceProductId: productId, durationSeconds: 3600 },
      })
      .expect(400);
    // 合法草稿（先不带产品）
    const draftRes = await req(ownerToken)
      .post("/api/v1/tenant/orders")
      .send({
        customerProfileId: customerId,
        requirement: {
          description: "找陪玩带排位，要求意识好",
          gameId,
          durationSeconds: 3600,
        },
      })
      .expect(201);
    const draft = draftRes.body.data as {
      id: string;
      status: string;
      orderNo: string;
    };
    ...
    // 缺产品不能确认
    await req(ownerToken)
      .post(`/api/v1/tenant/orders/${draft.id}/confirm`)
      .expect(400);

    // 带产品重新建草稿并确认
    const okRes = await req(ownerToken)
      .post("/api/v1/tenant/orders")
      .send({
        customerProfileId: customerId,
        requirement: {
    ...
    const confirmed = await req(ownerToken)
      .post(`/api/v1/tenant/orders/${ok.id}/confirm`)
      .expect(201);
    const view = confirmed.body.data as {
      status: string;
    ...
    const count = await client.outboxEvent.count({
      where: { tenantId: tenants[0]?.id, eventType: "order.confirmed" },
    });
  ...
  it("价格修改不影响已确认旧订单快照；新订单用新价", async () => {
    ...
    const firstView = (
      await req(ownerToken)
        .post(
          `/api/v1/tenant/orders/${(first.body.data as { id: string }).id}/confirm`,
        )
        .expect(201)
    ).body.data as {
      id: string;
    ...
    // 改价 7000
    const rules = await req(ownerToken)
      .get(`/api/v1/tenant/catalog/products/${productId}/pricing`)
      .expect(200);
    const ruleId = (rules.body.data as Array<{ id: string }>)[0]?.id as string;
    await req(ownerToken)
      .patch(`/api/v1/tenant/catalog/pricing/${ruleId}`)
      .send({ priceFen: "7000" })
      .expect(200);
    ...
    const secondView = (
      await req(ownerToken)
        .post(
          `/api/v1/tenant/orders/${(second.body.data as { id: string }).id}/confirm`,
        )
        .expect(201)
    ).body.data as {
      snapshot: Array<{ unitPriceFen: string }>;
    ...
    const oldView = (
      await req(ownerToken)
        .get(`/api/v1/tenant/orders/${firstView.id}`)
        .expect(200)
    ).body.data as {
      snapshot: Array<{ unitPriceFen: string }>;
    };
  ...
  it("金额预算字段必须为十进制字符串分；number 预算被拒", async () => {
    ...
    const view = (
      await req(ownerToken)
        .get(`/api/v1/tenant/orders/${(ok.body.data as { id: string }).id}`)
        .expect(200)
    ).body.data as {
  ...
  it("幂等：同 idempotencyKey 重复创建返回同一订单", async () => {
    ...
    const body = {
      customerProfileId: customerId,
      idempotencyKey: key,
      requirement: { description: "幂等单", gameId },
    };
    const a = (
      await req(ownerToken).post("/api/v1/tenant/orders").send(body).expect(201)
  ...
  it("取消 CONFIRMED 订单成功；重复取消 409", async () => {
    ...
    const order = await req(ownerToken)
      .post("/api/v1/tenant/orders")
      .send({
        customerProfileId: customerId,
        requirement: {
    ...
    await req(ownerToken)
      .post(
        `/api/v1/tenant/orders/${(order.body.data as { id: string }).id}/confirm`,
      )
      .expect(201);
    const cancelled = await req(ownerToken)
      .post(
        `/api/v1/tenant/orders/${(order.body.data as { id: string }).id}/cancel`,
      )
      .send({ reason: "客户改期" })
      .expect(201);
    expect((cancelled.body.data as { status: string }).status).toBe(
      "CANCELLED",
    );
    await req(ownerToken)
      .post(
  ...
  it("客户自助：CUSTOMER 绑定后 me/自助下单/我的订单", async () => {
    ...
    const acc = await client.tenantAccount.create({
      data: { tenantId, username: `cust_${suffix}`, passwordHash: ph },
    });
    await client.tenantAccountRole.create({
      data: { tenantId, tenantAccountId: acc.id, role: "CUSTOMER" },
    });
    await req(ownerToken)
      .post(`/api/v1/tenant/customers/${profileId}/account`)
    ...
    const myMe = await req(cToken)
      .get("/api/v1/tenant/customer/me")
      .expect(200);
    expect((myMe.body.data as { id: string }).id).toBe(profileId);
    expect((myMe.body.data as { name: string }).name).toBe("自助客户");

    const created = await req(cToken)
      .post("/api/v1/tenant/customer/orders")
      .send({
        requirement: {
          description: "客户自助下一单",
          serviceProductId: productId,
      ...
      .expect(201);
    expect((created.body.data as { customerName: string }).customerName).toBe(
      "自助客户",
    );
    expect(
      (created.body.data as { customerProfileId: string }).customerProfileId,
    ).toBe(profileId);

    const mine = await req(cToken)
      .get("/api/v1/tenant/customer/orders")
      .expect(200);
    const list = mine.body.data as Array<{
      id: string;
      customerProfileId: string;
    }>;
    expect(
      list.some((o) => o.id === (created.body.data as { id: string }).id),
    ).toBe(true);
