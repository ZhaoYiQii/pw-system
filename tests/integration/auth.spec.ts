...
} from "../../apps/api/src/modules/identity-access/domain/errors.js";
...
describe("identity-access (login / refresh / logout / audience)", () => {
  ...
  it("平台登录成功并签发可验证的 access token", async () => {
    ...
    const principal = await service.verifyAccess(bundle.accessToken, [
      "pw-platform",
    ]);
  ...
  it("错误密码/未知账号抛 InvalidCredentialsError", async () => {
    await expect(
      service.loginPlatform(`admin_${suffix}`, "wrong"),
    ).rejects.toBeInstanceOf(InvalidCredentialsError);
    await expect(
      service.loginPlatform(`nobody_${suffix}`, PW),
    ).rejects.toBeInstanceOf(InvalidCredentialsError);
  });

  it("停用账号被拒绝", async () => {
    await expect(
      service.loginPlatform(`disabled_${suffix}`, PW),
  ...
  it("门店登录 principal 带 tenantId；跨 audience 拒绝", async () => {
    ...
    await expect(
      service.verifyAccess(bundle.accessToken, ["pw-platform"]),
    ).rejects.toThrow();
    await expect(
      service.verifyAccess(bundle.accessToken, ["pw-tenant"]),
  ...
  it("refresh 旋转：旧 refresh token 失效，新 token 可用", async () => {
    ...
    await expect(
      service.refresh(first.refreshToken, "platform"),
  ...
  it("logout 后 refresh token 失效", async () => {
    ...
    await expect(
      service.refresh(bundle.refreshToken, "platform"),
    ).rejects.toBeInstanceOf(InvalidRefreshTokenError);
  });
  ...
  it("伪造/篡改 token 被拒绝", async () => {
    ...
    await expect(
      service.verifyAccess(forged, ["pw-platform"]),
    ).rejects.toThrow();
  });
});
