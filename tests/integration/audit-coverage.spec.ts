...
describe("C1 audit coverage: 关键写操作统一写入 audit_logs", () => {
  ...
  beforeAll(async () => {
    ...
    const playerAcc = await client.tenantAccount.create({ data: { tenantId, username: "player", passwordHash: hash } });
    await client.tenantAccountRole.create({ data: { tenantId, tenantAccountId: playerAcc.id, role: "PLAYER" } });
    const player = await client.playerProfile.create({
      data: { tenantId, name: "审计陪玩", tenantAccountId: playerAcc.id }
    });
    const game = await client.game.create({ data: { tenantId, name: "审计游戏" } });
    const product = await client.serviceProduct.create({ data: { tenantId, gameId: game.id, name: "审计产品" } });
    await client.pricingRule.create({
      data: { tenantId, serviceProductId: product.id, durationSeconds: 3600, priceFen: BigInt(20000), playerCostFen: BigInt(5000) }
    });
    await client.playerSkill.create({ data: { tenantId, playerId: player.id, gameId: game.id } });

    await client.platformAccount.create({
      data: { username: `audit_platform_${suffix}`, passwordHash: hash, role: "PLATFORM_SUPER_ADMIN" }
    });
    ...
    async function login(kind: "platform" | "tenant", username: string, password: string, code?: string): Promise<string> {
      const body: Record<string, string> = { kind, username, password };
      if (code) body.tenantCode = code;
      const res = await request(app.getHttpServer()).post("/api/v1/auth/login").send(body).expect(201);
  ...
  afterAll(async () => {
    if (client) {
      ...
      await client.platformAccount.deleteMany({ where: { username: `audit_platform_${suffix}` } });
  ...
  function req(token: string) {
    ...
    return {
      ...
      post: (u: string, b?: unknown) => request(app.getHttpServer()).post(u).set(h).send(b ?? {}),
      patch: (u: string, b?: unknown) => request(app.getHttpServer()).patch(u).set(h).send(b ?? {})
    };
  }

  async function auditActions(): Promise<string[]> {
    const list = (await req(ownerToken).get("/api/v1/tenant/audit").expect(200)).body.data as Array<{ action: string }>;
  ...
  it("订单→派单→场次→核算主链路全部写审计", async () => {
    ...
    const products = (await req(ownerToken).get(`/api/v1/tenant/catalog/products?gameId=${gameId}`).expect(200)).body.data as Array<{ id: string }>;
    const productId = products[0]?.id as string;

    const created = (await req(ownerToken)
      .post("/api/v1/tenant/orders", {
        customerProfileId: customerId,
        requirement: { description: "审计链路订单", serviceProductId: productId, durationSeconds: 3600 }
      })
      .expect(201)).body.data as { id: string };

    const confirmed = (await req(ownerToken).post(`/api/v1/tenant/orders/${created.id}/confirm`).expect(201)).body.data as { status: string };
    expect(confirmed.status).toBe("CONFIRMED");

    await req(ownerToken).post(`/api/v1/tenant/orders/${created.id}/publish`).expect(201);
    await req(playerToken).post(`/api/v1/tenant/player/orders/${created.id}/applications`, { note: "报名审计单" }).expect(201);

    const apps = (await req(ownerToken).get(`/api/v1/tenant/orders/${created.id}/applications`).expect(200)).body.data as Array<{ id: string }>;
    const appId = apps[0]?.id as string;
    await req(ownerToken).post(`/api/v1/tenant/orders/${created.id}/applications/${appId}/shortlist`, { shortlisted: true }).expect(201);
    const assigned = (await req(ownerToken)
      .post(`/api/v1/tenant/orders/${created.id}/assignment`, { applicationId: appId })
      .expect(201)).body.data as { id: string };
    expect(assigned.id).toBeTruthy();

    const start = (await req(playerToken).post(`/api/v1/tenant/orders/${created.id}/session/start`).expect(201)).body.data as { id: string };
  ...
  it("结算批次全流程写审计", async () => {
    ...
    const batch = (await req(ownerToken).post("/api/v1/tenant/settlements").expect(201)).body.data as { id: string };
    await req(financeToken).post(`/api/v1/tenant/settlements/${batch.id}/items`, { earningIds: [earningId] }).expect(201);
    await req(financeToken).post(`/api/v1/tenant/settlements/${batch.id}/review`).expect(201);
    await req(financeToken).post(`/api/v1/tenant/settlements/${batch.id}/approve`).expect(201);
    await req(ownerToken).post(`/api/v1/tenant/settlements/${batch.id}/pay`).expect(201);
  ...
  it("配置保存/回滚、费率、平台功能开关与登录失败写审计", async () => {
    await req(ownerToken).post("/api/v1/tenant/config", { config: VALID_CONFIG }).expect(201);
    await req(ownerToken).post("/api/v1/tenant/config", { config: VALID_CONFIG }).expect(201);
    await req(ownerToken).post("/api/v1/tenant/config/rollback").expect(201);

    await req(ownerToken).post("/api/v1/tenant/finance-rules/store-cut", { storeCutBp: 2500 }).expect(201);
    await req(platformToken).patch(`/api/v1/platform/tenants/${tenantId}/finance-rules`, { platformFeeBp: 400 }).expect(200);
    await req(platformToken)
      .post(`/api/v1/platform/tenants/${tenantId}/entitlements`, { featureKey: "addon.ai_requirement_parser", enabled: true })
      .expect(201);

    // 登录失败（有效账号+错误密码）应产生 auth.login_failed
    await request(app.getHttpServer())
      .post("/api/v1/auth/login")
      .send({ kind: "tenant", tenantCode, username: "boss", password: "wrong-password" })
      .expect(401);
