...
const document = JSON.parse(readFileSync(path.resolve(here, "../../openapi.json"), "utf8")) as {
  paths: Record<string, Record<string, unknown>>;
};

function responseSchema(methodPath: string, method: string, status: string): Record<string, unknown> {
  ...
  if (!schema) throw new Error(`missing ${method.toUpperCase()} ${methodPath} ${status} schema`);
  return schema;
}
...
function schemaAt(
  ...
  for (const segment of segments) {
    if (current === null || typeof current !== "object") throw new Error(`cannot descend ${String(segment)}`);
    current = (current as Record<string, unknown>)[String(segment)];
  }
  if (current === null || typeof current !== "object") throw new Error(`missing ${segments.join(".")}`);
  return current as Record<string, unknown>;
}

describe("OpenAPI 金额契约（MoneyFen 必须为十进制字符串）", () => {
  it("catalog 价格规则：输入输出 priceFen/playerCostFen 均为 string", () => {
    const createBody = document.paths["/api/v1/tenant/catalog/products/{productId}/pricing"]?.post as {
      requestBody?: { content?: Record<string, { schema?: Record<string, unknown> }> };
    };
    const bodyProps = schemaAt(
      createBody?.requestBody?.content?.["application/json"]?.schema as Record<string, unknown>,
      "properties"
    );
    expect(schemaAt(bodyProps, "priceFen")["type"]).toBe("string");
    expect(schemaAt(bodyProps, "playerCostFen")["type"]).toBe("string");

    const list = responseSchema("/api/v1/tenant/catalog/products/{productId}/pricing", "get", "200");
  ...
  it("orders 快照与预算金额为 string；接单大厅 unitPriceFen 为 string", () => {
    ...
    const hall = responseSchema("/api/v1/tenant/player/order-hall", "get", "200");
    const hallItemProps = schemaAt(hall, "properties", "data", "items", "properties");
    expect(schemaAt(hallItemProps, "unitPriceFen")["type"]).toBe("string");
  });

  it("finance split-preview 入参与输出、陪玩财务余额均为 string", () => {
    const preview = responseSchema("/api/v1/tenant/finance-rules/split-preview", "post", "200");
    ...
    const previewBody = document.paths["/api/v1/tenant/finance-rules/split-preview"]?.post as {
      requestBody?: { content?: Record<string, { schema?: Record<string, unknown> }> };
    };
    const amount = schemaAt(
      previewBody?.requestBody?.content?.["application/json"]?.schema as Record<string, unknown>,
      "properties",
      "amountFen"
    );
    expect(amount["type"]).toBe("string");

    const finance = responseSchema("/api/v1/tenant/player/finance", "get", "200");
