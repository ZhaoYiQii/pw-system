...
describe("tenant isolation for Slice 5 order tables", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({ data: { code: `o5_a_${suffix}`, name: "A 店" } });
    const b = await owner.tenant.create({ data: { code: `o5_b_${suffix}`, name: "B 店" } });
    tenantAId = a.id;
    tenantBId = b.id;
    const customerA = await owner.customerProfile.create({ data: { tenantId: a.id, name: "A客" } });
    customerAId = customerA.id;
    const customerB = await owner.customerProfile.create({ data: { tenantId: b.id, name: "B客" } });
    const orderB = await owner.order.create({
      data: { tenantId: b.id, orderNo: `o-${suffix}`, customerProfileId: customerB.id }
    });
    orderBId = orderB.id;
    await owner.orderRequirement.create({ data: { tenantId: b.id, orderId: orderB.id, description: "B需求" } });
    await owner.orderEvent.create({
      data: { tenantId: b.id, orderId: orderB.id, eventType: "ORDER_CREATED", fromStatus: null, toStatus: "DRAFT" }
    });
  });
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.orderPriceSnapshot.deleteMany({ where: { tenantId: { in: tids } } });
      await owner.orderRequirement.deleteMany({ where: { tenantId: { in: tids } } });
  ...
  it("INSERT：A 店上下文写入 tenant_id=B 的订单被拒", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
        tx.order.create({ data: { tenantId: tenantBId, orderNo: "x", customerProfileId: customerAId } })
      ).rejects.toThrow();
    });
  });
  ...
  it("合法写入：A 店上下文可创建本店订单并可见", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      const created = await tx.order.create({
        data: { tenantId: tenantAId, orderNo: `oa-${suffix}`, customerProfileId: customerAId }
      });
  ...
  it("DELETE：A 店上下文无法删除 B 店订单", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(tx.order.delete({ where: { id: orderBId } })).rejects.toThrow();
    });
  });
});