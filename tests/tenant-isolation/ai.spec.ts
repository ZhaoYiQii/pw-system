...
describe("tenant isolation for Slice 10 AI tables", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({
      data: { code: `a10_a_${suffix}`, name: "A" },
    });
    const b = await owner.tenant.create({
      data: { code: `a10_b_${suffix}`, name: "B" },
    });
    tenantAId = a.id;
    tenantBId = b.id;
    const runB = await owner.aiRun.create({
      data: {
        tenantId: b.id,
        runType: "PARSE_REQUIREMENT",
        modelVersion: "v1",
        promptVersion: "v1",
      },
    });
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.aiSuggestion.deleteMany({
  ...
  it("A 店读不到 B 店 aiRun；删除被拒；跨租户插入被拒", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      expect((await tx.aiRun.findMany()).map((x) => x.id)).not.toContain(
        runBId,
      );
      await expect(
        tx.aiRun.delete({ where: { id: runBId } }),
      ).rejects.toThrow();
    });
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
