...
describe("tenant isolation for Slice 6 dispatch tables", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({
      data: { code: `d6_a_${suffix}`, name: "A 店" },
    });
    const b = await owner.tenant.create({
      data: { code: `d6_b_${suffix}`, name: "B 店" },
    });
    tenantAId = a.id;
    tenantBId = b.id;
    ...
    const pubB = await owner.dispatchPublication.create({
      data: { tenantId: b.id, orderId: orderB.id },
    });
    pubBId = pubB.id;
  });
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.dispatchPublication.deleteMany({
        where: { tenantId: { in: tids } },
      });
      const ordersB = await owner.order.findMany({
        where: { tenantId: { in: tids } },
      });
  ...
  it("INSERT：A 店上下文写入 tenant_id=B 的发布被拒", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      const ob = await owner.order.findFirst({
        where: { tenantId: tenantBId },
      });
      await expect(
        tx.dispatchPublication.create({
          data: { tenantId: tenantBId, orderId: ob?.id as string },
        }),
      ).rejects.toThrow();
    });
  });
  ...
  it("DELETE：A 店上下文无法删除 B 店发布", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
        tx.dispatchPublication.delete({ where: { id: pubBId } }),
      ).rejects.toThrow();
    });
  });
});
