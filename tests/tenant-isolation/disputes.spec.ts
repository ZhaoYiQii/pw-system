...
describe("tenant isolation for Slice 9 dispute/audit", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({
      data: { code: `d9_a_${suffix}`, name: "A 店" },
    });
    const b = await owner.tenant.create({
      data: { code: `d9_b_${suffix}`, name: "B 店" },
    });
    tenantAId = a.id;
    tenantBId = b.id;
    ...
    const d = await owner.dispute.create({
      data: {
        tenantId: b.id,
        orderId: orderB.id,
        playerId: pb.id,
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.disputeEvent.deleteMany({
  ...
  it("A 店读不到 B 店 dispute/audit；删除被拒；本店插入被拒(tenant=B)", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      expect((await tx.dispute.findMany()).map((x) => x.id)).not.toContain(
        disputeBId,
      );
      expect((await tx.auditLog.findMany()).length).toBe(0);
      await expect(
        tx.dispute.delete({ where: { id: disputeBId } }),
      ).rejects.toThrow();
    });
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
