...
describe("tenant isolation for Slice 8 ledger/settlement", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({
      data: { code: `l8_a_${suffix}`, name: "A 店" },
    });
    const b = await owner.tenant.create({
      data: { code: `l8_b_${suffix}`, name: "B 店" },
    });
    tenantAId = a.id;
    tenantBId = b.id;
    ...
    const earningB = await owner.earning.create({
      data: {
        tenantId: b.id,
        playerId: pb.id,
        orderId: orderB.id,
        amountFen: BigInt(2000),
      },
    });
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.manualPaymentRecord.deleteMany({
        where: { tenantId: { in: tids } },
      });
      await owner.settlementItem.deleteMany({
        where: { tenantId: { in: tids } },
      });
      ...
      await owner.ledgerAccount.deleteMany({
        where: { tenantId: { in: tids } },
      });
  ...
  it("SELECT：A 店读不到 B 店 earning；DELETE 被拒；INSERT tenant_id=B 被拒", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      expect((await tx.earning.findMany()).map((e) => e.id)).not.toContain(
        earningBId,
      );
      await expect(
        tx.earning.delete({ where: { id: earningBId } }),
      ).rejects.toThrow();
    });
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
