...
describe("tenant isolation for Slice 7 session tables", () => {
  ...
  beforeAll(async () => {
    ...
    const a = await owner.tenant.create({
      data: { code: `s7_a_${suffix}`, name: "A 店" },
    });
    const b = await owner.tenant.create({
      data: { code: `s7_b_${suffix}`, name: "B 店" },
    });
    tenantAId = a.id;
    tenantBId = b.id;
    const cb = await owner.customerProfile.create({
    ...
    const orderB = await owner.order.create({
      data: {
        tenantId: b.id,
        orderNo: `s7o-${suffix}`,
        customerProfileId: cb.id,
      },
    });
    const pb = await owner.playerProfile.create({
      data: { tenantId: b.id, name: "B玩" },
    });
    const sessionB = await owner.serviceSession.create({
      data: { tenantId: b.id, orderId: orderB.id, playerId: pb.id },
    });
  ...
  afterAll(async () => {
    if (owner) {
      ...
      await owner.evidenceAsset.deleteMany({
        where: { tenantId: { in: tids } },
      });
      await owner.sessionAdjustment.deleteMany({
        where: { tenantId: { in: tids } },
      });
      const sessions = await owner.serviceSession.findMany({
        where: { tenantId: { in: tids } },
      });
      for (const s of sessions)
        await owner.sessionEvent.deleteMany({ where: { sessionId: s.id } });
      await owner.serviceSession.deleteMany({
        where: { tenantId: { in: tids } },
      });
      const orders = await owner.order.findMany({
        where: { tenantId: { in: tids } },
      });
      for (const o of orders)
        await owner.orderEvent.deleteMany({ where: { orderId: o.id } });
  ...
  it("SELECT：A 店上下文读不到 B 店场次；INSERT tenant_id=B 被拒；本店可见可删", async () => {
    ...
    await withTenantContext(runtime, tenantAId, async (tx) => {
      const created = await tx.serviceSession
        .create({
          data: {
            tenantId: tenantAId,
  ...
  it("DELETE：A 店上下文无法删除 B 店场次", async () => {
    await withTenantContext(runtime, tenantAId, async (tx) => {
      await expect(
        tx.serviceSession.delete({ where: { id: sessionBId } }),
      ).rejects.toThrow();
    });
  });
});
